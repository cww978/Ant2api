...
4. MANDATORY apply_patch Tool Invocation for Code & File Changes:
   - When modifying, creating, editing, or patching code or files, you MUST invoke the \`apply_patch\` (or editing) tool function with the patch passed as the tool argument!
   - ABSOLUTELY NEVER output \`*** Begin Patch\`, \`*** Update File\`, or \`\`\`patch / \`\`\`diff markdown code blocks into your conversational text!
   - The user's IDE client ONLY applies changes to local files and renders the "N files changed [Review]" diff bar when you invoke the \`apply_patch\` function call. Printing markdown text in chat will NOT modify any files on the user's disk!`;
      if (baseInstructions) {
