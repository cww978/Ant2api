...
项目采用全新的 **React 18 + Next.js** 现代化暗黑科技风管理后台，实现了 **管理后台与反向代理服务的双端口彻底分离**，支持在 Web 控制台中动态控制反向代理服务的启动、停止、重启与热配置。
...
│ 🔑 管理员密码:   ant2api_admin                             │
│ 📦 包含: React 管理后台前端 + /api/admin/* 控制 API            │
└──────────────────────────────┬──────────────────────────────┘
                               │
                控制启停 / 重启 / 动态调优
                               │
